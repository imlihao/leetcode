...
class Codec
{
  ...
  private:
    void serial(TreeNode *root, ostringstream &ist)
    {
        if (root != nullptr)
        {
            ist << root->val << ' ';
            serial(root->left, ist);
            serial(root->right, ist);
        }
        else
        {
            ist << "*" << ' ';
    ...
    TreeNode *deserial(istringstream &ist)
    {
        string str;
        ist >> str;
        if (str == "*")
        ...
        {
            TreeNode *td = new TreeNode(stoi(str));
            td->left = deserial(ist);
            td->right = deserial(ist);
            return td;
        }
    }
};
